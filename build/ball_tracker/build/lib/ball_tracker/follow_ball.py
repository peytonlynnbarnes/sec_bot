...
class FollowBall(Node):
    ...
    def listener_callback(self, msg):
        f = self.filter_value
        # Correct interpretation: msg.y = offset, msg.x = size
        self.target_x = self.target_x * f + msg.y * (1 - f)
        self.target_size = self.target_size * f + msg.x * (1 - f)
        self.lastrcvtime = time.time()
        self.get_logger().info(f'Received ball position: offset x={self.target_x}, size={self.target_size}')

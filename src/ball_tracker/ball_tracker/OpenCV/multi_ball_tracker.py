...
def capture_frames():
    ...
    while True:
        ...
        if ret and frame_queue.qsize() < 2:
            frame_queue.put(
                cv2.UMat(frame)
            )  # converts the frame to UMat for GPU acceleration
...
MAX_DISTANCE = (
    60  # maximum distance a ball can move between frames before it's considered "new"
)
TRACK_TIMEOUT = (
    1.5 * cv2.getTickFrequency()
)  # if a ball disappears for 1.5 seconds, remove it
...
while True:
    ...
    for detection in current_detections:
        ...
        for track_id, track in tracks.items():
            distance = np.linalg.norm(
                np.array(detection) - np.array(track.prediction[:2])
            )
    ...
    stale_tracks = [
        tid for tid, t in tracks.items() if (current_tick - t.last_seen) > TRACK_TIMEOUT
    ]
